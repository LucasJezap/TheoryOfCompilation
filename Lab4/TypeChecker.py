...
class TypeChecker(NodeVisitor):
    ...
    def visit_Transposition(self, node):
        ...
        if not isinstance(type1, ArrayT):
            print(f"Line {node.line}: Can not transpose {type1}, type mismatch")
            return ArrayT(2, AnyT, (None,None))
        if type1.dims != 2: 
            print(f"Line {node.line}: Can not transpose {type1}, dimension mismatch")
    ...
    def visit_BinExpr(self, node):
        ...
        if op[0] == '.':
            ...
            if isinstance(type1, ArrayT) and isinstance(type2, ArrayT):
                ...
                if type3 == AnyT:
                    print(f'Line {node.line}: Can not apply {op} for {type1.eltype} and {type2.eltype}, expression will result in any type')
                return ArrayT(type1.dims, type3, type1.size)
            elif isinstance(type1, ArrayT):
                type3 = aaa[op][type1.eltype][type2]
                if type3 == AnyT:
                    print(f'Line {node.line}: Can not apply {op} for {type1.eltype} and {type2}, expression will result in any type')
                return ArrayT(type1.dims, type3, type1.size)
            elif isinstance(type2, ArrayT):
                type3 = aaa[op][type1][type2.eltype]
                if type3 == AnyT:
                    print(f'Line {node.line}: Can not apply {op} for {type1} and {type2.eltype}, expression will result in any type')
            ...
            if type3 == AnyT:
                print(f'Line {node.line}: Can not apply {op} for {type1} and {type2}, expression will result in any type')
            return type3

    def visit_Id(self, node):
        type1 = self.current_scope.get(node.id)
        if type1 == None:
            print(f'Line {node.line}: Variable can not be found in current scope')
            return AnyT
        return type1
    ...
    def visit_IfStmt(self, node):
        ...
        if condt != BoolT:
            print(f'Line {node.line}: If must have condition resolving to boolean value, but got {condt}')
    ...
    def visit_ForLoop(self, node):
        ...
        if type1 != 'range':
            print(f'Line {node.line}: For loop must be iterating over range, but got {type1}')
    ...
    def visit_WhileLoop(self, node):
        ...
        condt = self.visit(node.cond)
        if condt != BoolT:
            print(f'Line {node.line}: While loop must have condition resolving to boolean value, but got {condt}')
    ...
    def visit_Vector(self, node):
        ...
        if any(eltype != t for t in types):
            if isinstance(eltype, ArrayT):
                print(f"Line {node.line}: Inconsistant vector lengths, choosing first length to fit dimension")
                return ArrayT(eltype.dims + 1, eltype.eltype, (len(types),) + eltype.size)
            print(f'Line {node.line}: Inconsistant vector value types, choosing any as vector base type')
    ...
    def visit_Break(self, node):
        if self.loop_count == 0:
            print(f"Line {node.line}: Line {node.line}: Using break outside of loop")
        return None
    ...
    def visit_Ref(self, node):
        ...
        if targett == StringT and len(node.indices) != 1:
            print(f"Line {node.line}: Indexing string with {len(node.indices)} dimensions")
            return IntT
        if isinstance(targett, ArrayT): 
            if len(node.indices) != targett.dims:
                print(f"Line {node.line}: Indexing {targett.dims}d array with {len(node.indices)} dimensions")
